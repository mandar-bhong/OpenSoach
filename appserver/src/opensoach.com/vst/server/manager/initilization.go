...
var SUB_MODULE_NAME = "VST.Endpoint.Manager"
...
func InitilizeModues(dbconfig *gmodels.ConfigDB) error {
	...
	errPrepareConfig, masterConfigSetting := pcmgr.PrepareMasterConfiguration(dbconfig, masterConfigData, gmodels.PRODUCT_TYPE_VST)
...
func initModules(configSetting *gmodels.ConfigSettings) error {
	...
	prodTaskConfig.DefaultQueue = gmodels.VST_SERVER_DEFAULT_TASK_QUEUE
	...
	go func() {
		err := prodTaskCtx.StartWorker("VST_SERVER_WORKER")
