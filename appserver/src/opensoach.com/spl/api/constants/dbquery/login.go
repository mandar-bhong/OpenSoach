...
const QUERY_GET_USER_LOGIN_INFO = `SELECT usr_name,fname,lname,urole.urole_code FROM spl_master_user_tbl usr LEFT JOIN spl_master_usr_cpm_tbl ucpm ON usr.id = ucpm.user_id_fk LEFT JOIN spl_master_usr_details_tbl usrd ON usr.id = usrd.usr_id_fk  LEFT JOIN spl_master_user_role_tbl urole ON urole.id = ucpm.urole_id_fk WHERE ucpm.cpm_id_fk = ? AND  usr.id = ?`

const Query_GET_OSU_USER_LOGIN_INFO = `SELECT usr_name,fname,lname,urole.urole_code
FROM spl_master_user_tbl usr  
LEFT JOIN spl_master_usr_details_tbl usrd ON usr.id = usrd.usr_id_fk
left join spl_master_user_role_tbl urole on urole.id = usr.urole_id_fk
WHERE usr.id = ?`
