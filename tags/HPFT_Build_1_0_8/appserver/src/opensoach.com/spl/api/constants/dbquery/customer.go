...
const QUERY_GET_CUST_SERVICE_POINTS = `select cpm.id as id,prod.prod_code as prod_code, 
(select count(id)  from spl_master_servicepoint_tbl where cpm_id_fk = cpm.id ) as spcount
