...
let selectQueries = new Map([
    ...
    ['outputType', "select conf from conf_tbl where conf_type_code='OUTPUT_TYPE'"],
    ['medicineType', "select conf from conf_tbl where conf_type_code='MEDICINE_TYPE'"],
    ['doctorName', "select usr_id,usr_name, urole_name,fname,lname from usr_tbl  where urole_name in ('External Doctor','Internal Doctor')"],
    ['intakeType', "select conf from conf_tbl where conf_type_code='INPUT_TYPE'"],
    ['orderType', "select conf from conf_tbl where conf_type_code='DOCTOR_ORDER_TYPE'"],
    ...
    ["patient_admission_tbl_update", "update patient_admission_tbl set patient_uuid=?, cpm_id=?, patient_reg_no=?, bed_no=?, status=?, sp_uuid=?, dr_incharge=?, admitted_on=?, discharged_on=?,updated_by=?, updated_on=?, sync_pending=?, client_updated_at=? where uuid=?"],
    ...
    ["patient_admission_tbl_insert", "insert into patient_admission_tbl (uuid, cpm_id, patient_uuid, patient_reg_no, bed_no, status, sp_uuid, dr_incharge, admitted_on, discharged_on, updated_by, updated_on, sync_pending,client_updated_at) values ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"],
