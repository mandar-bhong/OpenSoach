...
public class DatabaseManager {
    ...
    private static boolean _isInitCompleted;
    ...
    public static <T extends IDBRowMapper, K extends Object> List<K> SelectAll(T queryModel, K dataModel, String sortFilter) {
        List selectList = new ArrayList<K>();

        if (! (queryModel instanceof IDBOutputFormatter )) {
            throw new IncompatibleClassChangeError("IDBOutputFormatter interface not implemented");
        }

        SQLiteDatabase db = DatabaseManager.openDatabase();

        Cursor cursor = db.query(DBTableName(queryModel), queryModel.SelectColumn(), null, null, null, null, ((IDBOutputFormatter)queryModel).OrderBy(sortFilter));
    ...
    public static <T extends IDBRowMapper, K extends Object> List<K> SelectByFilter(T queryModel, K dataModel, String filter,String sortFilter) {
        ...
        Cursor cursor = db.query(tableName, queryModel.SelectColumn(), queryModel.WhereFilter(filter),
                queryModel.FilterArgs(dataModel, filter), null, null, ((IDBOutputFormatter)queryModel).OrderBy(sortFilter), null);
