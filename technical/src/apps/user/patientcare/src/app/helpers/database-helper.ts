...
let selectQueries = new Map([
    ...
    ["monitorTxnList", "select schedule.conf,txn.schedule_uuid,txn_data,txn_date  from action_txn_tbl as txn left join schedule_tbl as schedule on txn.schedule_uuid = schedule.uuid where schedule.conf_type_code = 'Monitor' order by txn_date asc"],
    ...
    ["doctors_orders_tbl_insert", "insert into doctors_orders_tbl (uuid, admission_uuid, doctor_id, doctors_orders, document_uuid, document_name, doctype, updated_by, updated_on, sync_pending, client_updated_at) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "],
    ["doctors_orders_tbl_update", "update doctors_orders_tbl set admission_uuid=?, doctor_id=?, doctors_orders=?, document_uuid=?, document_name=?, doctype=?, updated_by=?, updated_on=?, sync_pending=?, client_updated_at=? where uuid=? "],
