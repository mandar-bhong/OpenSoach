...
where patient.cpm_id_fk = :cpmid and patient.updated_on > :updatedon',	
	'{
  "userdevice": "select cast(greatest(patient.updated_on, coalesce(upmm.updated_on,0),coalesce(upmm1.updated_on,0),coalesce(upmm2.updated_on,0)) as datetime) as updated_on,patient.id,patient.cpm_id_fk,patient.patient_reg_no,patient.uuid,patient.fname,patient.lname,patient.mob_no,patient.date_of_birth,patient.age,patient.blood_grp,patient.gender,patient.client_updated_at,patient.created_on,patient.updated_by from spl_hpft_patient_master_tbl patient inner join spl_hpft_patient_admission_tbl padmsn on padmsn.patient_id_fk = patient.id left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where patient.cpm_id_fk = :cpmid and (patient.updated_on > :updatedon or upmm.updated_on > :updatedon or upmm1.updated_on > :updatedon or upmm2.updated_on > :updatedon ) group by patient.id",
  "shareddevice": "select patient.* from spl_hpft_patient_master_tbl patient inner join spl_hpft_patient_admission_tbl padmsn on padmsn.patient_id_fk = patient.id where patient.cpm_id_fk = :cpmid and patient.updated_on > :updatedon"
	...
	'{
  "userdevice": "select cast(greatest(pconf.updated_on, coalesce(upmm.updated_on,0),coalesce(upmm1.updated_on,0),coalesce(upmm2.updated_on,0)) as datetime) as updated_on,padmsn.id as admission_id,pconf.uuid, padmsn.uuid as admission_uuid,pconf.conf_type_code,pconf.conf,pconf.start_date,pconf.end_date,pconf.status,pconf.updated_by,pconf.client_updated_at from spl_hpft_patient_conf_tbl pconf left join spl_hpft_patient_admission_tbl  padmsn on padmsn.id = pconf.admission_id_fk  left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where pconf.cpm_id_fk = :cpmid and (pconf.updated_on > :updatedon or upmm.updated_on > :updatedon or upmm1.updated_on > :updatedon or upmm2.updated_on > :updatedon ) group by pconf.id",
...
values 
(:uuid,:cpm_id_fk,(select id as patient_id_fk from spl_hpft_patient_master_tbl where uuid = :patient_uuid limit 1),:patient_reg_no,:bed_no,:status,(select sp_id_fk from spl_node_sp_tbl where uuid = :sp_uuid limit 1),:dr_incharge,:admitted_on,:discharged_on,TRIM(TRAILING "Z" FROM :client_updated_at),:updated_by)',
'update spl_hpft_patient_admission_tbl 
...
'{
  "userdevice": "select cast(greatest(pdetails.updated_on, coalesce(upmm.updated_on,0),coalesce(upmm1.updated_on,0),coalesce(upmm2.updated_on,0)) as datetime) as updated_on,padmsn.id as admission_id,pdetails.uuid,patient.uuid as patient_uuid,padmsn.uuid as admission_uuid, pdetails.age,pdetails.other_details,pdetails.person_accompanying,pdetails.updated_by,pdetails.client_updated_at from spl_hpft_patient_personal_details_tbl pdetails left join spl_hpft_patient_admission_tbl padmsn on padmsn.id = pdetails.admission_id_fk inner join spl_hpft_patient_master_tbl patient on patient.id = padmsn.patient_id_fk  left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where pdetails.cpm_id_fk = :cpmid and (pdetails.updated_on > :updatedon or upmm.updated_on > :updatedon or upmm1.updated_on > :updatedon or upmm2.updated_on > :updatedon ) group by pdetails.id",
...
'{
  "userdevice": "select cast(greatest(mdetails.updated_on, coalesce(upmm.updated_on,0),coalesce(upmm1.updated_on,0),coalesce(upmm2.updated_on,0)) as datetime) as updated_on,padmsn.id as admission_id,mdetails.uuid, patient.uuid as patient_uuid,padmsn.uuid as admission_uuid,mdetails.present_complaints,mdetails.reason_for_admission,mdetails.history_present_illness,mdetails.past_history,mdetails.treatment_before_admission,mdetails.investigation_before_admission,mdetails.family_history,mdetails.allergies,mdetails.personal_history,mdetails.updated_by,mdetails.client_updated_at from spl_hpft_patient_medical_details_tbl mdetails left join spl_hpft_patient_admission_tbl padmsn on padmsn.id = mdetails.admission_id_fk inner join spl_hpft_patient_master_tbl patient on patient.id = padmsn.patient_id_fk  left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where mdetails.cpm_id_fk = :cpmid and (mdetails.updated_on > :updatedon or upmm.updated_on > :updatedon or upmm1.updated_on > :updatedon or upmm2.updated_on > :updatedon ) group by mdetails.id",
...
'{
  "userdevice": "select cast(greatest(atxn.updated_on, coalesce(upmm.updated_on,0),coalesce(upmm1.updated_on,0),coalesce(upmm2.updated_on,0)) as datetime) as updated_on, padmsn.id as admission_id,atxn.uuid,padmsn.uuid as admission_uuid,pconf.uuid as schedule_uuid,atxn.txn_data,atxn.scheduled_time,atxn.txn_state,atxn.conf_type_code,atxn.runtime_config_data,atxn.updated_by,atxn.client_updated_at from spl_hpft_action_txn_tbl atxn left join spl_hpft_patient_admission_tbl padmsn on padmsn.id = atxn.admission_id_fk left join spl_hpft_patient_conf_tbl pconf on pconf.id = atxn.patient_conf_id_fk  left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where atxn.cpm_id_fk = :cpmid and (atxn.updated_on > :updatedon or upmm.updated_on > :updatedon or upmm1.updated_on > :updatedon or upmm2.updated_on > :updatedon ) group by atxn.id",
...
'{
  "userdevice": "select cast(greatest(doc_ordrs.updated_on, coalesce(upmm.updated_on,0),coalesce(upmm1.updated_on,0),coalesce(upmm2.updated_on,0)) as datetime) as updated_on,padmsn.id as admission_id,doc_ordrs.uuid,padmsn.uuid as admission_uuid,doc_ordrs.doctor_id_fk as doctor_id,doc_ordrs.doctors_orders,doc_ordrs.comment,doc_ordrs.ack_by, doc_ordrs.ack_time, doc_ordrs.status, doc_ordrs.order_created_time, doc_ordrs.order_type,doc.uuid as document_uuid,doc.name as document_name,doc.doctype,doc_ordrs.updated_by,doc_ordrs.client_updated_at from spl_hpft_doctors_orders_tbl doc_ordrs left join spl_hpft_patient_admission_tbl padmsn on padmsn.id = doc_ordrs.admission_id_fk left join spl_hpft_document_tbl doc on doc_ordrs.document_id_fk = doc.id  left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where doc_ordrs.cpm_id_fk = :cpmid and (doc_ordrs.updated_on > :updatedon or upmm.updated_on > :updatedon or upmm1.updated_on > :updatedon or upmm2.updated_on > :updatedon ) group by doc_ordrs.id",
...
'{
  "userdevice": "select cast(greatest(trtmnt.updated_on, coalesce(upmm.updated_on,0),coalesce(upmm1.updated_on,0),coalesce(upmm2.updated_on,0)) as datetime) as updated_on,padmsn.id as admission_id,trtmnt.uuid,padmsn.uuid  as admission_uuid,treatment_done,treatment_performed_time,details,post_observation,trtmnt.updated_by  from spl_hpft_treatment_tbl trtmnt inner join spl_hpft_patient_admission_tbl padmsn on padmsn.id = trtmnt.admission_id_fk left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where trtmnt.cpm_id_fk =:cpmid and (trtmnt.updated_on > :updatedon or upmm.updated_on > :updatedon or upmm1.updated_on > :updatedon or upmm2.updated_on > :updatedon ) group by trtmnt.id",
...
'{
  "userdevice": "select cast(greatest(precdoc.updated_on, coalesce(upmm.updated_on,0),coalesce(upmm1.updated_on,0),coalesce(upmm2.updated_on,0)) as datetime) as updated_on,prec.admission_id_fk as admission_id,prec.uuid as pathology_record_uuid,doc.uuid as document_uuid,doc.name as document_name,doc.doctype from spl_hpft_pathology_record_tbl prec left join spl_hpft_pathology_record_doc_tbl precdoc on precdoc.pathology_id_fk = prec.id left join spl_hpft_document_tbl doc on doc.id = precdoc.document_id_fk inner join spl_hpft_patient_admission_tbl padmsn on padmsn.id = prec.admission_id_fk  left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where (precdoc.updated_on > :updatedon or upmm.updated_on > :updatedon or upmm1.updated_on > :updatedon or upmm2.updated_on > :updatedon ) group by precdoc.pathology_id_fk",
...
'{
  "userdevice": "select cast(greatest(actn.updated_on, coalesce(upmm.updated_on,0),coalesce(upmm1.updated_on,0),coalesce(upmm2.updated_on,0)) as datetime) as updated_on,padmsn.id as admission_id,actn.uuid,padmsn.uuid as admission_uuid,pconf.uuid as schedule_uuid,actn.conf_type_code,actn.scheduled_time,actn.is_deleted,actn.updated_by,actn.client_updated_at  from spl_hpft_action_tbl actn inner join spl_hpft_patient_admission_tbl padmsn on padmsn.id = actn.admission_id_fk inner join spl_hpft_patient_conf_tbl pconf on pconf.id = actn.patient_conf_id_fk  left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where actn.cpm_id_fk =:cpmid and (actn.updated_on > :updatedon or upmm.updated_on > :updatedon or upmm1.updated_on > :updatedon or upmm2.updated_on > :updatedon ) group by actn.id",
