...
(:uuid,:cpm_id_fk, (select id as admission_id_fk from spl_hpft_patient_admission_tbl where uuid = :admission_uuid limit 1),(select id as doctor_id_fk from spl_hpft_document_tbl where uuid = :document_uuid limit 1),:doctors_orders,:comment,:ack_by,STR_TO_DATE(:ack_time ,"%Y-%m-%dT%T.%xZ"),:status,STR_TO_DATE(:order_created_time ,"%Y-%m-%dT%T.%xZ"),:order_type,(select id as document_id_fk from spl_hpft_document_tbl where uuid = :document_uuid limit 1),STR_TO_DATE(:client_updated_at ,"%Y-%m-%dT%T.%xZ"),:updated_by)','update spl_hpft_doctors_orders_tbl set cpm_id_fk = :cpm_id_fk,admission_id_fk = (select id as admission_id_fk from spl_hpft_patient_admission_tbl where uuid = :admission_uuid limit 1),doctor_id_fk = :doctor_id_fk,doctors_orders = :doctors_orders,comment = :comment,ack_by = :ack_by,ack_time = STR_TO_DATE(:ack_time ,"%Y-%m-%dT%T.%xZ"),status = :status,order_created_time = STR_TO_DATE(:order_created_time ,"%Y-%m-%dT%T.%xZ"),order_type = :order_type,document_id_fk = (select id as document_id_fk from spl_hpft_document_tbl where uuid = :document_uuid limit 1),client_updated_at=STR_TO_DATE(:client_updated_at ,"%Y-%m-%dT%T.%xZ"),updated_by = :updated_by where uuid = :uuid',2),
	...
	('treatment_tbl','2018-01-01 00:00:00','select count(*) as count from spl_hpft_treatment_tbl where uuid = ?','select count(*) as count, max(updated_on) as max_updated_on from spl_hpft_treatment_tbl where updated_on > ?','select trtmnt.uuid,padmsn.uuid  as admission_uuid,treatment_done,treatment_performed_time,details,post_observation,trtmnt.updated_by,trtmnt.updated_on 
...
left join spl_hpft_document_tbl doc on doc.id = tdoc.document_id_fk where trtmnt.updated_on > ?','insert_qry','update_qry',2),
	...
	('pathology_record_tbl','2018-01-01 00:00:00','select count(*) as count from spl_hpft_pathology_record_tbl where uuid = ?','select count(*) as count, max(updated_on) as max_updated_on from spl_hpft_pathology_record_tbl where updated_on > ?','select prec.uuid, padmsn.uuid as admission_uuid,test_performed,test_performed_time,test_result,comments,prec.updated_by,prec.updated_on 
