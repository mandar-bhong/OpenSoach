INSERT INTO `spl_node_report_template_tbl` (`id`, `report_code`, `report_desc`, `report_header`, `report_query_params`, `report_query`) VALUES
	...
	group by taskname'),
	(2, 'TASK_SUMMARY_SP', 'This will produce task list', '{"en": ["Service Point", "Service Point Category", "Completed Tasks", "Delayed Tasks"]}', '1','select 
sp.sp_name as sp_name,
spc.spc_name as spc_name,
...
where sp.cpm_id_fk = $WhereCpmIdValue$ and sp.sp_id_fk = ? and txn_date between ? and ? group by sp_name, spc_name'),
	(3, 'TASK_LIST_ALL', 'This will produce task list', '{"en": ["Service Point", "Task Name", "On Time", "Delayed", "Total"]}', '1','select locationname, taskname, count(ontime) as ontime, count(delay) as \'delayed\', count(total) as total from (
	select
