...
const QUERY_GET_SPL_MASTER_CUSTOMER_TABLE_TOTAL_FILTERED_COUNT = `SELECT count(*) as count FROM spl_master_customer_tbl cust INNER JOIN spl_master_cust_details_tbl custdetails ON custdetails.cust_id_fk = cust.id INNER JOIN spl_master_corp_tbl corp ON corp.id = cust.corp_id_fk $WhereCondition$`
