...
const QUERY_GET_SPL_MASTER_DEVICE_TABLE_TOTAL_FILTERED_COUNT = `Select count(*) as count From spl_master_customer_tbl cust
Inner Join spl_master_cust_prod_mapping_tbl cpm on cpm.cust_id_fk = cust.id
Inner Join spl_master_cpm_dev_mapping_tbl cpdm on cpdm.cpm_id_fk = cpm.id
Inner Join spl_master_device_tbl dev on dev.id = cpdm.dev_id_fk
Left Join spl_master_dev_details_tbl devd on devd.dev_id_fk = dev.id
Left Join spl_master_dev_status_tbl devstate on devstate.dev_id_fk = dev.id $WhereCondition$`
...
From spl_master_customer_tbl cust
Inner Join spl_master_cust_prod_mapping_tbl cpm on cpm.cust_id_fk = cust.id
Inner Join spl_master_cpm_dev_mapping_tbl cpdm on cpdm.cpm_id_fk = cpm.id
Inner Join spl_master_device_tbl dev on dev.id = cpdm.dev_id_fk
Left Join spl_master_dev_details_tbl devd on devd.dev_id_fk = dev.id
...
const QUERY_GET_DEV_ID_BY_CPM_ID = `Select dev_id_fk From spl_master_cpm_dev_mapping_tbl where cpm_id_fk = ?`
