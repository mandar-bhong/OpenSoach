...
inner join spl_node_dev_tbl dev on dev.dev_id_fk = devsp.dev_id_fk
left join spl_node_service_conf_tbl serv_conf on serv_conf.spc_id_fk = sp.spc_id_fk $WhereCondition$`

const QUERY_NODE_SP_TABLE_SELECT_BY_FILTER = `select sp.sp_id_fk,sp.sp_name,sp.spc_id_fk,spc.spc_name,devsp.dev_id_fk,dev.dev_name,serv_conf.id as id,sp.sp_state,sp.sp_state_since 
