...
INSERT INTO `spl_node_sync_config_tbl` (`store_name`, `updated_on`, `has_qry`, `select_count_qry`, `select_qry`, `insert_qry`, `update_qry`,`data_source`, `query_data`) VALUES
	('service_point_tbl',
	'2018-01-01 00:00:00',
	'select count(*) as count from spl_node_sp_tbl where uuid = ?','select count(*) as count, max(updated_on) as max_updated_on from spl_node_sp_tbl where cpm_id_fk = :cpmid and updated_on > :updatedon','select * from spl_node_sp_tbl where cpm_id_fk = :cpmid and updated_on > :updatedon','insert_qry','update_qry',2,
...
where patient.cpm_id_fk = :cpmid and patient.updated_on > :updatedon and padmsn.status = 1',	
	'select patient.* from spl_hpft_patient_master_tbl patient
inner join spl_hpft_patient_admission_tbl padmsn on padmsn.patient_id_fk = patient.id
where patient.cpm_id_fk = 3 and patient.updated_on > :updatedon and padmsn.status = 1',	
	...
	'select padmsn.id as admission_id,pconf.uuid, padmsn.uuid as admission_uuid,pconf.conf_type_code,pconf.conf,pconf.start_date,pconf.end_date,pconf.status,pconf.updated_by,pconf.updated_on,pconf.client_updated_at from spl_hpft_patient_conf_tbl pconf
...
'select padmsn.id as admission_id,pdetails.uuid,patient.uuid as patient_uuid,padmsn.uuid as admission_uuid, pdetails.age,pdetails.other_details,pdetails.person_accompanying,pdetails.updated_by,pdetails.updated_on,pdetails.client_updated_at from spl_hpft_patient_personal_details_tbl pdetails
left join spl_hpft_patient_admission_tbl padmsn on padmsn.id = pdetails.admission_id_fk
inner join spl_hpft_patient_master_tbl patient on patient.id = padmsn.patient_id_fk where pdetails.cpm_id_fk = :cpmid and pdetails.updated_on > :updatedon',
...
'select padmsn.id as admission_id,atxn.uuid,padmsn.uuid as admission_uuid,pconf.uuid as schedule_uuid,atxn.txn_data,atxn.scheduled_time,atxn.txn_state,atxn.conf_type_code,atxn.runtime_config_data,atxn.updated_by,atxn.updated_on,atxn.client_updated_at from spl_hpft_action_txn_tbl atxn
left join spl_hpft_patient_admission_tbl padmsn on padmsn.id = atxn.admission_id_fk
left join spl_hpft_patient_conf_tbl pconf on pconf.id = atxn.patient_conf_id_fk where atxn.cpm_id_fk = :cpmid and atxn.updated_on > :updatedon',
...
'select padmsn.id as admission_id,trtmnt.uuid,padmsn.uuid  as admission_uuid,treatment_done,treatment_performed_time,details,post_observation,trtmnt.updated_by,trtmnt.updated_on 
...
'select trtmnt.admission_id_fk as admission_id,trtmnt.uuid as treatment_uuid,doc.uuid as document_uuid,doc.name as document_name,doc.doctype,tdoc.updated_on from spl_hpft_treatment_tbl trtmnt
left join spl_hpft_treatment_doc_tbl tdoc on tdoc.treatment_id_fk = trtmnt.id
...
'select padmsn.id as admission_id,prec.uuid, padmsn.uuid as admission_uuid,test_performed,test_performed_time,test_result,comments,prec.updated_by,prec.updated_on 
...
'select prec.admission_id_fk as admission_id,prec.uuid as pathology_record_uuid,doc.uuid as document_uuid,doc.name as document_name,doc.doctype,precdoc.updated_on from spl_hpft_pathology_record_tbl prec
left join spl_hpft_pathology_record_doc_tbl precdoc on precdoc.pathology_id_fk = prec.id
...
'select padmsn.id as admission_id,actn.uuid,padmsn.uuid as admission_uuid,pconf.uuid as schedule_uuid,actn.conf_type_code,actn.scheduled_time,actn.is_deleted,actn.updated_on,actn.updated_by,actn.client_updated_at 
