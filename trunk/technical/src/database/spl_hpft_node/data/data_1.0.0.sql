...
INSERT INTO `spl_node_sync_config_tbl` (`store_name`, `updated_on`, `has_qry`, `select_count_qry`, `select_qry`, `insert_qry`, `update_qry`,`data_source`, `query_data`) VALUES
	('service_point_tbl',
	'2018-01-01 00:00:00',
	'select count(*) as count from spl_node_sp_tbl where uuid = ?','select count(*) as count, max(updated_on) as max_updated_on from spl_node_sp_tbl where cpm_id_fk = :cpmid and updated_on > :updatedon','select * from spl_node_sp_tbl where cpm_id_fk = :cpmid and updated_on > :updatedon','insert_qry','update_qry',2,'{"select": {"filters": ["cpm", "updatedon"]}}'),
	
	('conf_tbl',
	'2018-01-01 00:00:00',
	'select count(*) as count from spl_hpft_conf_tbl where uuid = ?','select count(*) as count, max(updated_on) as max_updated_on from spl_hpft_conf_tbl where cpm_id_fk = :cpmid and updated_on > :updatedon','select * from spl_hpft_conf_tbl where cpm_id_fk = :cpmid and updated_on > :updatedon','insert_qry','update_qry',
	2,
	...
	'select count(*) as count, max(updated_on) as max_updated_on from spl_hpft_patient_master_tbl where cpm_id_fk = :cpmid and updated_on > :updatedon',	
	'select * from spl_hpft_patient_master_tbl where cpm_id_fk = :cpmid and updated_on > :updatedon',	
	...
	values 
	(:uuid,:cpm_id_fk,:patient_reg_no,:fname,:lname,:mob_no,:age,:blood_grp,:gender, STR_TO_DATE(:client_updated_at ,"%Y-%m-%dT%T.%xZ"),:updated_by)',
	...
	'select count(*) as count, max(pconf.updated_on) as max_updated_on from spl_hpft_patient_conf_tbl pconf left join spl_hpft_patient_admission_tbl  padmsn on padmsn.id = pconf.admission_id_fk where pconf.cpm_id_fk = :cpmid and pconf.updated_on > :updatedon',
	'select pconf.uuid, padmsn.uuid as admission_uuid,pconf.conf_type_code,pconf.conf,pconf.start_date,pconf.end_date,pconf.status,pconf.updated_by,pconf.updated_on,pconf.client_updated_at from spl_hpft_patient_conf_tbl pconf
			left join spl_hpft_patient_admission_tbl  padmsn on padmsn.id = pconf.admission_id_fk where pconf.cpm_id_fk = :cpmid and pconf.updated_on > :updatedon',
...
values (:uuid,:cpm_id_fk,(select id as admission_id_fk from spl_hpft_patient_admission_tbl where uuid = :admission_uuid limit 1) ,:conf_type_code,:conf,STR_TO_DATE(:start_date ,"%Y-%m-%dT%T.%xZ"),STR_TO_DATE(:end_date ,"%Y-%m-%dT%T.%xZ"),:status,STR_TO_DATE(:client_updated_at ,"%Y-%m-%dT%T.%xZ"),:updated_by)','update spl_hpft_patient_conf_tbl set cpm_id_fk = :cpm_id_fk, admission_id_fk = (select id as admission_id_fk from spl_hpft_patient_admission_tbl where uuid = :admission_uuid limit 1), conf_type_code = :conf_type_code, conf = :conf, start_date= STR_TO_DATE(:start_date ,"%Y-%m-%dT%T.%xZ"), end_date= STR_TO_DATE(:end_date ,"%Y-%m-%dT%T.%xZ"),status=:status,client_updated_at=STR_TO_DATE(:client_updated_at ,"%Y-%m-%dT%T.%xZ"), updated_by = :updated_by WHERE uuid = :uuid',2,'{"select": {"filters": ["cpm", "updatedon"]}}'),
	
	('patient_admission_tbl',
	'2018-01-01 00:00:00',
	'select count(*) as count from spl_hpft_patient_admission_tbl where uuid = ?',
	'select count(*) as count, max(padmsn.updated_on) as max_updated_on from spl_hpft_patient_admission_tbl padmsn 
...
'select padmsn.uuid,patient.uuid as patient_uuid,padmsn.patient_reg_no,padmsn.bed_no,padmsn.status,sp.uuid as sp_uuid,padmsn.dr_incharge,padmsn.admitted_on,padmsn.discharged_on,padmsn.updated_by,padmsn.updated_on, padmsn.client_updated_at from spl_hpft_patient_admission_tbl padmsn 
left join spl_hpft_patient_master_tbl patient on patient.id = padmsn.patient_id_fk
left join spl_node_sp_tbl sp on sp.sp_id_fk = padmsn.sp_id_fk where padmsn.cpm_id_fk = :cpmid and padmsn.updated_on > :updatedon',
...
'{"select": {"filters": ["cpm", "updatedon"]}}'),
	...
	'select count(*) as count, max(pdetails.updated_on) as max_updated_on from spl_hpft_patient_personal_details_tbl pdetails
...
'select pdetails.uuid,patient.uuid as patient_uuid,padmsn.uuid as admission_uuid, pdetails.age,pdetails.other_details,pdetails.person_accompanying,pdetails.updated_by,pdetails.updated_on,pdetails.client_updated_at from spl_hpft_patient_personal_details_tbl pdetails
left join spl_hpft_patient_admission_tbl padmsn on padmsn.id = pdetails.admission_id_fk
inner join spl_hpft_patient_master_tbl patient on patient.id = padmsn.patient_id_fk where pdetails.cpm_id_fk = :cpmid and pdetails.updated_on > :updatedon',
...
values (:cpm_id_fk,(select id as patient_id from spl_hpft_patient_master_tbl where uuid = :patient_uuid limit 1),(select id as admission_id_fk from spl_hpft_patient_admission_tbl where uuid = :admission_uuid limit 1),:uuid,:age,:other_details,:person_accompanying,STR_TO_DATE(:client_updated_at ,"%Y-%m-%dT%T.%xZ"),:updated_by)',
...
'{"select": {"filters": ["cpm", "updatedon"]}}'),
	...
	'select count(*) as count, max(mdetails.updated_on) as max_updated_on from spl_hpft_patient_medical_details_tbl mdetails
...
'{"select": {"filters": ["cpm", "updatedon"]}}'),
	...
	'select count(*) as count, max(atxn.updated_on) as max_updated_on from spl_hpft_action_txn_tbl atxn
...
'select atxn.uuid,padmsn.uuid as admission_uuid,pconf.uuid as schedule_uuid,atxn.txn_data,atxn.scheduled_time,atxn.txn_state,atxn.conf_type_code,atxn.runtime_config_data,atxn.updated_by,atxn.updated_on,atxn.client_updated_at from spl_hpft_action_txn_tbl atxn
left join spl_hpft_patient_admission_tbl padmsn on padmsn.id = atxn.admission_id_fk
left join spl_hpft_patient_conf_tbl pconf on pconf.id = atxn.patient_conf_id_fk where atxn.cpm_id_fk = :cpmid and atxn.updated_on > :updatedon',
...
'select trtmnt.uuid,padmsn.uuid  as admission_uuid,treatment_done,treatment_performed_time,details,post_observation,trtmnt.updated_by,trtmnt.updated_on 
...
'select prec.uuid, padmsn.uuid as admission_uuid,test_performed,test_performed_time,test_result,comments,prec.updated_by,prec.updated_on 
from spl_hpft_pathology_record_tbl prec
inner join spl_hpft_patient_admission_tbl padmsn on padmsn.id = prec.admission_id_fk 
where prec.cpm_id_fk = :cpmid and prec.updated_on > :updatedon','insert_qry','update_qry',
2,
'{"select": {"filters": ["cpm", "updatedon"]}}'),
...
'SELECT count(usr.id) as count , max(usr.updated_on) as max_updated_on
	...
	'select count(*) as count, max(actn.updated_on) as max_updated_on from spl_hpft_action_tbl actn
...
'select actn.uuid,padmsn.uuid as admission_uuid,pconf.uuid as schedule_uuid,actn.conf_type_code,actn.scheduled_time,actn.is_deleted,actn.updated_on,actn.updated_by,actn.client_updated_at 
...
values
(:uuid,:cpm_id_fk,(select id as admission_id_fk from spl_hpft_patient_admission_tbl where uuid = :admission_uuid limit 1),(select id as patient_conf_id_fk from spl_hpft_patient_conf_tbl where uuid = :schedule_uuid limit 1),:conf_type_code,CAST(:scheduled_time AS DATETIME),:is_deleted,STR_TO_DATE(:client_updated_at ,"%Y-%m-%dT%T.%xZ"),:updated_by)','update spl_hpft_action_tbl set cpm_id_fk = :cpm_id_fk,admission_id_fk = (select id as admission_id_fk from spl_hpft_patient_admission_tbl where uuid = :admission_uuid limit 1),patient_conf_id_fk = (select id as patient_conf_id_fk from spl_hpft_patient_conf_tbl where uuid = :schedule_uuid limit 1),conf_type_code = :conf_type_code,scheduled_time = CAST(:scheduled_time AS DATETIME),is_deleted = :is_deleted,client_updated_at = STR_TO_DATE(:client_updated_at ,"%Y-%m-%dT%T.%xZ"),updated_by = :updated_by where uuid = :uuid',
2,
'{"select": {"filters": ["cpm", "updatedon"]}}');
