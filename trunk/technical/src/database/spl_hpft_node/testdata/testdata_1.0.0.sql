...
(uuid,cpm_id_fk,admission_id_fk,doctor_id_fk,doctors_orders,document_id_fk,updated_by)
values
(:uuid,:cpm_id_fk, (select id as admission_id_fk from spl_hpft_patient_admission_tbl where uuid = :admission_uuid limit 1),:doctor_id_fk,:doctors_orders,(select id as document_id_fk from spl_hpft_document_tbl where uuid = :document_uuid limit 1),:updated_by)','update spl_hpft_doctors_orders_tbl set cpm_id_fk = :cpm_id_fk,admission_id_fk = (select id as admission_id_fk from spl_hpft_patient_admission_tbl where uuid = :admission_uuid limit 1),doctor_id_fk = :doctor_id_fk,doctors_orders = :doctors_orders,document_id_fk = (select id as document_id_fk from spl_hpft_document_tbl where uuid = :document_uuid limit 1),updated_by = :updated_by where uuid = :uuid'),

	('document_tbl', '2018-01-01 00:00:00', 'select count(*) as count from spl_hpft_document_tbl where uuid = ?', 'select_count_qry', 'select_qry', 'insert into spl_hpft_document_tbl (uuid,cpm_id_fk,persisted,updated_by,client_updated_at) values (:uuid,:cpm_id_fk,0,:updated_by,:client_updated_at)', 'update_qry'),
