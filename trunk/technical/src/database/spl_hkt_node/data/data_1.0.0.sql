INSERT INTO `spl_node_report_template_tbl` (`id`, `report_code`, `report_desc`, `report_header`, `report_query_params`, `report_query`) VALUES
	...
	group by taskname'),
	(2, 'TASK_SUMMARY_SP', 'This will produce task list', '{"en": ["Task Name", "On Time", "Delayed", "Total"]}', '1','select sp.sp_name,spc.spc_name from spl_node_sp_tbl sp
inner join spl_node_sp_category_tbl spc on sp.spc_id_fk = spc.id
where sp.cpm_id_fk = $WhereCpmIdValue$ and sp_id_fk = ?'),
	(3, 'TASK_LIST_ALL', 'This will produce task list', '{"en": ["Task Name", "On Time", "Delayed", "Total"]}', '1','select taskname, count(ontime) as ontime, count(delay) as \'delayed\', count(total) as total from (
	select
	txn_data ->> \'$.taskname\' as taskname,
	  case status
			when 1 then 1
	...
	group by taskname'),
	(4, 'TASK_SUMMARY_ALL', 'This will produce task list', '{"en": ["Task Name", "On Time", "Delayed", "Total"]}', '1','select sp.sp_name,spc.spc_name from spl_node_sp_tbl sp
inner join spl_node_sp_category_tbl spc on sp.spc_id_fk = spc.id
where sp.cpm_id_fk = $WhereCpmIdValue$');