...
const QUERY_SPL_NODE_DASHBOARD_COMPLAINT_SUMMARY = `select  complaint_state,count(*) as count from spl_vst_sp_complaint_tbl $WhereCondition$ group by complaint_state`
...
from spl_vst_sp_complaint_tbl
$WhereCondition$
group by month,year`

const QUERY_GET_TOP_COMPLAINTS = `select id,complaint_title,raised_on,complaint_state,severity from spl_vst_sp_complaint_tbl $WhereCondition$
