...
configPath = `E:\Workspace\Project\OSSVN\servicepointlive\trunk\technical\src\appserver\src\github.com\RichardKnop\machinery\example\config.yml`
	if configPath != "" {
...
func MyTask(str string) (string, error) {

	log.INFO.Printf("Received : %s \n", str)
	t1val := &t1{}
	err := json.Unmarshal([]byte(str),t1val)

	if err != nil {
		log.INFO.Printf("Json Convert error : %s \n", err.Error())
	}

	log.INFO.Print( t1val)

	//value := fmt.Sprintf("%#v",t1val)

	log.INFO.Printf("Converted data : %#v \n", t1val)

	return str,nil
}

type t1 struct {
		myt1 string `json:"myt1"`
		myt2 int `json:"myt2"`
	}

func Deseri(dataType string, value interface{}) (reflect.Value,error){
...
func send() error {
	...
	var initTasks = func() {
		...
		t1Task = tasks.Signature{
			...
			Args: []tasks.Arg{
				{
					Type:  "string",
					Value: `{"myt1":"This is test data","myt2":6}`,
				},
			},
		}
	...
	asyncResult, err = server.SendTaskWithContext(ctx, &t1Task)
	results, err = asyncResult.Get(time.Duration(time.Millisecond * 5))
	if err != nil {
		return fmt.Errorf("Could not send t1Task: %s", err.Error())
	}
	log.INFO.Printf("Custome task result %s \n", tasks.HumanReadableResults(results))
