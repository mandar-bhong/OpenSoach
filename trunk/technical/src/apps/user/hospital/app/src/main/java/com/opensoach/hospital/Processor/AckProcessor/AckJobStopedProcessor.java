...
public class AckJobStopedProcessor implements IProcessor {
    ...
    public PacketProcessResultModel Process(PacketDecodeResultModel resultModel) {
        ...
        try {
            ...
            if ( !(dataModel instanceof PacketJobStopDataModel)) {
                AppLogger.getInstance().Log(AppLogger.LogLevel.Error, "AckJobStarted: Request is not of PacketJobStopDataModel. Packet:" + resultModel.JSONPacket);
                return packetProcessResultModel;
            }

            PacketJobStopDataModel packetJobStartDataModel = (PacketJobStopDataModel) dataModel;
            ...
            if (ackModel.Payload.Ack == Constants.RESPONSE_ACK_SUCCESS) {
                dbJobCardTableRowModelItem.setJobCardId(packetJobStartDataModel.JobId);

                //DatabaseManager.DeleteByFilter(new DBJobCardTableQueryModel(), dbJobCardTableRowModelItem, DBJobCardTableQueryModel.DELETE_BY_ID_FILTER);
                ...
                packetProcessResultModel.UINotifierModel.DataProcessStatergyID = ApplicationConstants.UI_PROCESSING_STATERGY_JOB_STOPED_SUCCESS;
                ...
                packetProcessResultModel.UINotifierModel = new AppNotificationModelBase();
                packetProcessResultModel.UINotifierModel.Data = dbJobCardTableRowModelItem;
                packetProcessResultModel.UINotifierModel.DataProcessStatergyID = ApplicationConstants.UI_PROCESSING_STATERGY_JOB_STOPED_FAILURE;
            ...
            AppLogger.getInstance().Log(ex,"Error occured in AckJobStopedProcessor");
        }
