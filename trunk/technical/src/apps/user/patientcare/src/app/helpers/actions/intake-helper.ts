...
import { Medicinefrequency, DayTimes, ActionItems, Monitorfrequency, Intakefrequency } from "~/app/models/ui/action-model";
...
export class IntakeHelper extends ActionHelper {
    ...
    createActionAfterXTimeinterval(receivedDate, IntakeSchedularData, i) {
    ...
    generateActionsOnSpecificTime(receivedDate, IntakeSchedularData, i) {
        ...
        if (receivedActionDate.getTime() == this.startDateWithoutHours.getTime()) {
            ...
            for (let h = 0; h < IntakeSchedularData.conf.specificTimes.length; h++) {
                const receivedSpecificTime = Math.floor(IntakeSchedularData.conf.specificTimes[h] * 60);
