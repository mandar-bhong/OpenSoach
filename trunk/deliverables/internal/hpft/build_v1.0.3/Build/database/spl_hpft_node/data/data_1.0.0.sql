...
where patient.cpm_id_fk = :cpmid and patient.updated_on > :updatedon and padmsn.status = 1',	
	'{
  "userdevice": "select  if(patient.updated_on > upmm.updated_on,patient.updated_on,upmm.updated_on) as updated_on,patient.id,patient.cpm_id_fk,patient.patient_reg_no,patient.uuid,patient.fname,patient.lname,patient.mob_no,patient.date_of_birth,patient.age,patient.blood_grp,patient.gender,patient.client_updated_at,patient.created_on,patient.updated_by from spl_hpft_patient_master_tbl patient inner join spl_hpft_patient_admission_tbl padmsn on padmsn.patient_id_fk = patient.id left join spl_hpft_user_patient_monitor_mapping upmm on upmm.sp_id_fk = padmsn.sp_id_fk and upmm.patient_id_fk = padmsn.patient_id_fk left join spl_hpft_user_patient_monitor_mapping upmm1 on upmm1.sp_id_fk = padmsn.sp_id_fk and upmm1.patient_id_fk is null left join spl_hpft_user_patient_monitor_mapping upmm2 on upmm2.sp_id_fk is null and upmm2.patient_id_fk is null where patient.cpm_id_fk = :cpmid and (patient.updated_on > :updatedon or upmm.updated_on > :updatedon ) and padmsn.status = 1 group by patient.id",
  "shareddevice": "select patient.* from spl_hpft_patient_master_tbl patient inner join spl_hpft_patient_admission_tbl padmsn on padmsn.patient_id_fk = patient.id where patient.cpm_id_fk = :cpmid and patient.updated_on > :updatedon and padmsn.status = 1"
	}',	
...
'{"shareddevice":{"filters":{"select":["cpm","updatedon"]}},"userdevice":{"filters":{"select":["cpm", "updatedon", "usrid"]}}}'),
	...
	'select padmsn.id as admission_id,count(*) as count, max(padmsn.updated_on) as max_updated_on from spl_hpft_patient_admission_tbl padmsn 
left join spl_hpft_patient_master_tbl patient on patient.id = padmsn.patient_id_fk
left join spl_node_sp_tbl sp on sp.sp_id_fk = padmsn.sp_id_fk where padmsn.cpm_id_fk = :cpmid and padmsn.status = 1',
	'{
  "userdevice": "select padmsn.id as admission_id,padmsn.uuid,padmsn.updated_on from spl_hpft_patient_admission_tbl padmsn  left join spl_hpft_patient_master_tbl patient on patient.id = padmsn.patient_id_fk left join spl_node_sp_tbl sp on sp.sp_id_fk = padmsn.sp_id_fk where padmsn.cpm_id_fk = :cpmid and padmsn.status = 1",
  "shareddevice": "select padmsn.id as admission_id,padmsn.uuid,padmsn.updated_on from spl_hpft_patient_admission_tbl padmsn  left join spl_hpft_patient_master_tbl patient on patient.id = padmsn.patient_id_fk left join spl_node_sp_tbl sp on sp.sp_id_fk = padmsn.sp_id_fk where padmsn.cpm_id_fk = :cpmid and padmsn.status = 1"
 }',
	'insert_qry',
	'update_qry',
	2,
