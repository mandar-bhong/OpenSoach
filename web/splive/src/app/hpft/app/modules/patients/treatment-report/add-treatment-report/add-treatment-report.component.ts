import { Component, EventEmitter, OnInit, Output, ViewChild } from '@angular/core';
import { FormControl, FormGroup, Validators } from '@angular/forms';
import { GetUUID } from '../../../../../../shared/helpers/get-uuid';
import { AppNotificationService } from '../../../../../../shared/services/notification/app-notification.service';
import { EDITABLE_RECORD_STATE, EditRecordBase, FORM_MODE } from '../../../../../../shared/views/edit-record-base';
import { TreatmentReportRequest } from '../../../../models/api/treatment-report-request';
import { PatientService } from '../../../../services/patient.service';

@Component({
  selector: 'app-add-treatment-report',
  templateUrl: './add-treatment-report.component.html',
  styleUrls: ['./add-treatment-report.component.css']
})
export class AddTreatmentReportComponent extends EditRecordBase implements OnInit {
  @Output() restForm = new EventEmitter();
  @ViewChild('fileUploadInput') fileUploadInputVariable: any;
  closeForm() {
    throw new Error("Method not implemented.");
  }
  uploadDocumentsIsEnable: boolean;
  doc_Array: File;
  uploadedDocuments: any[] = [];
  documentFile: any = null;
  constructor(private patientService: PatientService,
    private appNotificationService: AppNotificationService) {
    super();
  }

  ngOnInit() {
    this.createControls();
    this.iconCss = 'medical-icon-i-physical-therapy'
    this.pageTitle = "Treatment Report";
    this.subTitle = 'Add Treatment Report Details';
    this.recordState = EDITABLE_RECORD_STATE.ADD;
    this.setFormMode(FORM_MODE.EDITABLE);
    this.showBackButton = false;
    // this.callbackUrl = params['callbackurl'];
  }
  createControls(): void {
    this.editableForm = new FormGroup({
      treatmentdone: new FormControl('', [Validators.required]),
      details: new FormControl('', [Validators.required]),
      postobservation: new FormControl(),
      treatmentperformedtime: new FormControl('', [Validators.required])
    });
  }
  // code block for save 
  onFormSubmit() {
    if (this.editableForm.valid) {
      const treatmentReportRequest = new TreatmentReportRequest();
      treatmentReportRequest.treatmentdone = this.editableForm.get('treatmentdone').value;
      treatmentReportRequest.details = this.editableForm.get('details').value;
      treatmentReportRequest.postobservation = this.editableForm.get('postobservation').value;
      treatmentReportRequest.treatmentperformedtime=this.editableForm.get('treatmentperformedtime').value;
      const uuid = new GetUUID();
      treatmentReportRequest.uuid = uuid.getUuid();
      treatmentReportRequest.admissionid = this.patientService.admissionid;
      treatmentReportRequest.documentuuidlist = [];
      if (this.uploadedDocuments.length > 0) {
        this.uploadedDocuments.forEach((item) => {
          treatmentReportRequest.documentuuidlist.push(item.documentid);
        });
      }
      this.patientService.addTeatmentReportData(treatmentReportRequest).subscribe(reportsPayloadResponse => {
        if (reportsPayloadResponse) {
          if (reportsPayloadResponse.issuccess) {
            this.appNotificationService.success('Report Added Successfully');
            this.restForm.emit(0);
          }
        }
      });

    }
  }
  uploadDocuments(): void {
    const formdata = new FormData();
    // code block for appending value to form data

    // formdata.append('UUID', uuid.getUuid());
    formdata.append('file', this.doc_Array, this.doc_Array.name);
    this.patientService.uploadReportsDocuments(formdata).subscribe(documentsPayloadResponse => {
      if (documentsPayloadResponse) {
        if (documentsPayloadResponse.issuccess) {
          this.appNotificationService.success('Document Uploaded Successfully');
          // tslint:disable-next-line:max-line-length
          this.uploadedDocuments.push({ 'documentid': documentsPayloadResponse.data.uuid, 'documentname': this.doc_Array.name });
          this.doc_Array = null;
          this.fileUploadInputVariable.nativeElement.value = '';
        }
      }
    });
  }

  onChange(event: any) {
    this.documentFile = [].slice.call(event.target.files);
    this.doc_Array = this.documentFile[0];
    if (this.documentFile.length > 0) {
      // document size validations
    }
  }// end of fucntion.

  cancle() {
    // emiting 1 for rest.
    this.restForm.emit(1);
  }
}


